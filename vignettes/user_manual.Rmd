...
There are three different ways of using the **emeScheme**. You can either use ist as a web-app at [https://rmkrug.shinyapps.io/dmd_app/](https://rmkrug.shinyapps.io/dmd_app/), as a web-app locally, or from the R prompt programatically. A comparison of these three methods can be found in the "Develop and use the `dmdScheme`" vignette in the **dmdScheme** package. Here, we will only describe how use the scheme from the R prompt, as the steps in the web-app (on the internet or locally) are easily understandable after going through this description.

## Working programatically via R

So the first thing to do is get and load the **emeScheme** R package. This includes to linstall and load the **dmdScheme** package from CRAN and than install the **emeScheme**  scheme package and the accompanying R package.
...
* **MdBibliometric** <span style="color:red">TODO</span>
* **MdAuthor** <span style="color:red">TODO</span>
...
This worksheet (together with MdAuthor) contains the bibliometric metadata, i.e. the metadata related to authorship, licenses, keywords, etc. Most of the fields are self explantory.   

## MdAuthors worksheet

This sheet contains everybody involved to such a degree in the measurement, curation and  other aspects involving the data gathering itself as well as the creation of this data package (datasets + metadata + other accompanying documents). Every scientist who publishes, should be encouraged to get a unique ID. One of the most widely used one is the [ORCID ID](https://orcid.org). This ID has to be entered in the column `orcid`. The last column contains the rol person has played in this process. The role is defined by [CRediT system, the Contributor Roles Taxonomy](https://casrai.org/credit/) at. One author can have more than one role.
