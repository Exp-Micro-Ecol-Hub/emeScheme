...
You can choose to work via a web based application or by writing and running code in R. In either case you will enter the metadata in a supplied spreadsheet (Googlesheets or Excel).


## Working via the web based application

There are three different ways of using the **emeScheme**. You can either use ist as a web-app at [https://rmkrug.shinyapps.io/dmd_app/](https://rmkrug.shinyapps.io/dmd_app/), as a web-app locally, or from the R prompt programatically. A comparison of these three methods can be found in the "Develop and Use the dmdScheme" vignette in the **dmdScheme** package. Here, we will only describe how use the scheme from the R prompt, as the steps in the web-app (on the internet or locally) are easily understandable from this description.

## Working programatically via R

So the first thing to do is get and load the **emeScheme** R package. This includes to linstall and load the **dmdScheme** package from CRAN and than install the **emeScheme**  scheme package

To install the **dmdScheme** package, you do
...
Now you have to load the **emeScheme** package. This package is a scheme package which can be downloaded from the [scheme repository](https://github.com/Exp-Micro-Ecol-Hub/dmdSchemeRepository). It contains the scheme definition, examples, and an R package which enhances the functionality. Everything is installed into a temporary library which is deleted when you quit R.
...
2. Download and install the scheme definition for the `emeScheme` and install it in a temporary scheme library by running `scheme_install()`
3. load the accompanying package **emeScheme** and activate the scheme emeScheme by calling `library("emeScheme")`

This has to be dona each time you load the package in a new R session, as the session library is only temporary.

To make this library permanent, run
...
If you are told there are problems with opening the spreadsheet then try `open_new_spreadsheet(format = FALSE)`
