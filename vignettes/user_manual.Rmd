...
So the first thing to do is get and load the **emeScheme** R package:
...
1. Download and install the scheme definition for the `emeScheme` and install it in a temporary scheme library by running `scheme_install("emeScheme", "0.9.5")`
2. activate the new scheme definition by calling `scheme_use("emeScheme", "0.9.5")`

This wil be done each time you load the package in a new session, as the session library is only temporary.
...
For details on the managing of schemes, see [vignette R r_package_introduction or on the web](https://exp-micro-ecol-hub.github.io/dmdScheme/r_package_introduction.html).
...
1. **Please don't not use any formulas! Only enter values directly! Formulas can (and likely will) cause strange messages later on!**
