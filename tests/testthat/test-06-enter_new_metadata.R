context("enter_new_metadata(schemeName = \"emeScheme\")")
...
test_that(
  ...
  {
    expect_equal(
      object = suppressWarnings(enter_new_metadata(schemeName = "emeScheme", file = fn, .skipBrowseURL = TRUE)),
      expected = fn,
    )
  }
)
...
test_that(
  ...
  {
    expect_equal(
      object = suppressWarnings(enter_new_metadata(schemeName = "emeScheme", file = fn, overwrite = TRUE, .skipBrowseURL = TRUE)),
      expected = fn,
    )
  }
)
...
test_that(
  ...
  {
    expect_true(
      object = file.exists( suppressWarnings(enter_new_metadata(schemeName = "emeScheme", file = NULL, open = TRUE, overwrite = FALSE, .skipBrowseURL = TRUE) ) )
    )
  }
)
...
test_that(
  ...
  {
    expect_known_output(
      object = suppressWarnings((nfn <- enter_new_metadata(schemeName = "emeScheme", file = fn, open = TRUE, overwrite = FALSE, verbose = FALSE, .skipBrowseURL = TRUE))),
